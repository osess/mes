...
class ModelFormAdminView(ModelAdminView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.valid_forms():
            ...
            #modify by xxd, set technology,manufactureplan cannot be add
            # import pdb; pdb.set_trace()
            from django.contrib.auth.models import Group
            gs = Group.objects.filter(name="超级组")
            is_super_group = True if gs and gs[0] in self.request.user.groups.all() else False
            
            ct = ContentType.objects.get_for_model(self.new_obj)
            technology = None
            if ct.model == 'technology':
                technology = self.new_obj
            if ct.model == 'operationgroup':
                technology = self.new_obj.technology
            if not is_super_group and ct.model == 'operation':
                technology = self.new_obj.operation_group.technology
            if not is_super_group and ct.model == 'operationattribute':
                technology = self.new_obj.operation.operation_group.technology
            if technology:
                if technology.technology_workflow_status == 2 or technology.technology_workflow_approved:
                    return HttpResponseRedirect(self.model_admin_url('changelist'))

            manufactureplan = None
            if ct.model == 'manufactureplan':
                manufactureplan = self.new_obj
            if ct.model == 'manufactureitemgroup':
                manufactureplan = self.new_obj.manufactureplan
            if manufactureplan:
                if manufactureplan.manufactureplan_workflow_status == 2 or manufactureplan.manufactureplan_workflow_approved:
                    return HttpResponseRedirect(self.model_admin_url('changelist'))

            self.save_models()
            self.save_related()

            #modify by xxd, cannot add the exist knife
            if ct.model == 'knife':
                kas = self.new_obj.knife_attributes.all()
                ks = ct.model_class().objects.all()
                for ka in kas:
                    ks = ks.filter(
                        knife_attributes__attribute=ka.attribute,
                        knife_attributes__value=ka.value
                    )
                if ks.count() > 1:
                    self.new_obj.delete()
                    return HttpResponseRedirect(self.model_admin_url('changelist'))
...
class CreateAdminView(ModelFormAdminView):
    ...
    def get_context(self):
        new_context = {
            'title': _('Add') + _(force_unicode(self.opts.verbose_name)),
        }
    ...
    def get_breadcrumb(self):
        ...
        item = {'title': _('Add') + _(force_unicode(self.opts.verbose_name))}
...
class UpdateAdminView(ModelFormAdminView):
    ...
    def get_response(self, *args, **kwargs):
        #modify by xxd, set technology,manufactureplan cannot be change
        from django.contrib.auth.models import Group
        gs = Group.objects.filter(name="超级组")
        is_super_group = True if gs and gs[0] in self.request.user.groups.all() else False
        
        technology = None
        if ContentType.objects.get_for_model(self.org_obj).model == 'technology':
            technology = self.org_obj
        if ContentType.objects.get_for_model(self.org_obj).model == 'operationgroup':
            technology = self.org_obj.technology
        if not is_super_group and ContentType.objects.get_for_model(self.org_obj).model == 'operation':
            technology = self.org_obj.operation_group.technology
        if ContentType.objects.get_for_model(self.org_obj).model == 'operationattribute':
            technology = self.org_obj.operation.operation_group.technology
        if technology:
            if technology.technology_workflow_status == 2 or technology.technology_workflow_approved:
                return HttpResponseRedirect(self.model_admin_url('changelist'))

        manufactureplan = None
        if ContentType.objects.get_for_model(self.org_obj).model == 'manufactureplan':
            manufactureplan = self.org_obj
        if ContentType.objects.get_for_model(self.org_obj).model == 'manufactureitemgroup':
            manufactureplan = self.org_obj.manufactureplan
        if manufactureplan:
            if manufactureplan.manufactureplan_workflow_status == 2 or manufactureplan.manufactureplan_workflow_approved:
                return HttpResponseRedirect(self.model_admin_url('changelist'))
