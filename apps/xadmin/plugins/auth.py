...
class UserAdmin(object):
    def execute(self, instance):
        from person.models import UserPlainPassword
        upp = UserPlainPassword.objects.get(username=instance.username)
        return "<a data-res-uri='/xadmin/person/userplainpassword/%d/detail/' data-edit-uri='/xadmin/person/userplainpassword/%d/update/' class='details-handler' rel='tooltip' title='%s'>%s</a>" %(upp.id, upp.id, _("show password"), _("show password"))
    execute.short_description = _("show password")
    execute.allow_tags = True
    execute.is_column = True


    change_user_password_template = None
    list_display = ('username', 'last_name', 'first_name', 'is_staff', 'is_active', 'date_joined', 'last_login', 'execute')
    ...
    from settings import CAN_ADD_SUPER_USER
    if not CAN_ADD_SUPER_USER:
        exclude = ['is_superuser', 'user_permissions', 'last_login', 'date_joined', 'email']
